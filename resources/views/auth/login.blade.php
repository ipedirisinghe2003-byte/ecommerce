                ...
                <div class="w-full lg:w-1/2">
                    <div class="bg-gray-50 border rounded-lg p-8 shadow-sm">
                        ...
                        <form method="POST" action="{{ route('login') }}" class="space-y-4">
                            ...
                            <div>
                                <input id="email" name="email" type="email" value="{{ old('email') }}" placeholder="Username or email *" class="w-full border p-3 rounded focus:outline-none focus:border-primary transition @error('email') border-red-500 @enderror" required autofocus autocomplete="username" />
                                @error('email')
                            ...
                            <div>
                                <input id="password" name="password" type="password" placeholder="Password" class="w-full border p-3 rounded focus:outline-none focus:border-primary transition @error('password') border-red-500 @enderror" required autocomplete="current-password" />
                                @error('password')
                                    <span class="mt-2 block text-sm text-red-500">{{ $message }}</span>
                                @enderror
                            </div>
